...
<div class="panel panel-default">
 ...
 <!-- <div class="alert alert-info" style="font-size:20px;">用户后台地址与管理员一样，不同账户拥有不同权限。</div>-->
  ...
  <table class="table table-hover">
    ...
    <tbody>
    	<c:forEach var="loginrecord" items="${loginrecords}">    	
		  <tr id="${loginrecord.id}">
			<td>${loginrecord.id}</td>
			<td>${loginrecord.username}</td>
			
			<td><fmt:formatDate value='${loginrecord.lastlogintime}' pattern='yyyy-MM-dd HH:mm:ss'/></td>
			<td>${loginrecord.lastloginip}</td>
			<td><fmt:formatDate value='${loginrecord.createusertime}' pattern='yyyy-MM-dd HH:mm:ss'/></td>
			<td align="center"> 
			  <a class="btn btn-default btn-sm" onclick="updatePwd(${userid})" title="修改密码"><i class="icon-remove"></i>修改密码</a>
			  	  		  
			  <a class="btn btn-danger btn-sm" onclick="deleteSalesman(${loginrecord.id})" title="删除"><i class="icon-remove"></i>删除</a>
			</td>
		  </tr>	
    	</c:forEach>
    </tbody>
...
function updatePwd(id){
		...
		function(inputValue) {
			...
			$.ajax({
			  type: "POST",
			  url: "admin/Salesman/salesmanEditPwd?password="+inputValue+"&id="+id,
...
function deleteSalesman(id){
	...
	function() {
		 $.ajax({
          type: "POST",
          url: "admin/Salesman/salesmanDelete?id="+id,
          success: function(result) {
        	  result = eval("("+result+")");
            if (result.result == "0") {
			 $("#tr" + id).remove();
